<!-- BEGIN LOOM loom.core.setup.codex -->
# Loom Coordination Protocol

- Work from explicit success criteria before coding.
- Start from a Loom brief: `loom task start "<task>"`.
- If you are resuming, run `loom task resume` before editing.
...
- When ownership changes, hand off with `loom handoff --to claude-code`.
...
<!-- END LOOM loom.core.setup.codex -->

<!-- selvedge:begin -->
## Selvedge Security Plugin

Selvedge is installed. Before adding or upgrading any dependency, check it:

```
selvedge check <ecosystem> <name@version>
# e.g. selvedge check npm lodash@4.17.21
```

Read the `decision_hint`:
- `proceed` — safe to install.
- `warn` — explain the advisory to the user, suggest the safe version, ask for confirmation.
- `block` — refuse. Explain the advisory. Propose a safe alternative.

Never install dependencies silently. Never omit vulnerability details.

For code security review:
```
selvedge scan code --path <dir>            # SAST, secrets, completeness
selvedge triage <finding-id> <state>       # resolve a finding
```

Skill reference files are in `.codex/skills/selvedge/`.
<!-- selvedge:end -->

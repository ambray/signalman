...
- New scenario: `scenarios/ospiri-silo-poc/`
  ...
  - Assertions:
    ...
    - Helper process's `Global\OspiriSiloPocMutex_<pid>` is invisible from outside the silo
    - Exit code = 0
- New scenario: `scenarios/ospiri-silo-poc-containers-on/`
...
### Phase 6: Ospiri Correlator V1-V3 Isolation Scenarios — NEW 2026-04-17
...
- `scenarios/ospiri-v1-silo-isolation/` — agent launches a monitored AI tool,
  promotes it to a silo, validates `\BaseNamedObjects` isolation + ETW visibility
- `scenarios/ospiri-v1-appcontainer-compose/` — confirms silo composes with
...
- `scenarios/ospiri-v2-registry-deny/` — validates OspriReg driver denies
  writes to `HKLM\...\Run`, `...\Services`, IFEO, Scheduled Tasks
- `scenarios/ospiri-v2-network-egress/` — validates OspriNet WFP callout
...
- `scenarios/ospiri-v3-fs-isolation/` — OspriFs minifilter denies read of
  secret paths (`%LOCALAPPDATA%\Microsoft\Vault`, browser credential caches)
- `scenarios/ospiri-v3-handle-filter/` — OspriObj denies cross-silo handle opens
- `scenarios/ospiri-v3-complex-app/` — contained Claude Cowork scenario:

...
**2026-04-24 strategic shift**: Signalman is being repositioned from "Ospiri
...
agent in the loop). Ospiri V1–V3 scenarios move to `examples/` and no longer
gate Signalman releases.

**2026-04-24 change**: Phase 6.2 V2 "Gated Silo" scenarios have landed ahead of
schedule. `ospiri-v2-registry-deny` ships with ETW assertions, and
`ospiri-v2-network-egress` ships with WFP-live assertions against the OspriNet
classify callback. A bonus `ospiri-v2-network-torture` scenario was added for
...
`buildBackendList` or `signalman.yaml`. This aligns with the Ospiri correlator's
...
- **Audit C7**: cleanup/orphan reaper. The scenario teardown guard is closed
  ...
  errors. Remaining C7 scope is broader orphan/GC policy: no orphan reaper for
  VMs spawned by failed `vm_create`, no GC on `.signalman/recordings/`, and no
...
- **A4 — Replace cursor-restrict as the lead README scenario.** Today's
  ...
  `ospiri-v2-network-egress`. Or ship the missing files plus real
...
Their delivery cadence is decoupled from Signalman semvers — Ospiri team
owns the schedule.
...
### Ospiri Correlator V1–V3 Isolation Scenarios
...
- `examples/ospiri-v1-silo-isolation/` — agent launches a monitored AI
  tool, promotes it to a silo, validates `\BaseNamedObjects` isolation +
  ETW visibility.
- `examples/ospiri-v1-appcontainer-compose/` — confirms silo composes with
...
- `examples/ospiri-v2-registry-deny/` — landed with real ETW assertions
  (commits `1c57ff5`, `7073372`); driver-last-path-captured assertion
  retired in `72debe6` after Sprint 60.11 DIAG removal.
- `examples/ospiri-v2-network-egress/` — landed pre-WFP (`8df8736`), then
  ...
  (`2600333`).
- `examples/ospiri-v2-network-torture/` — bonus scenario added in
...
- `examples/ospiri-v3-fs-isolation/` — OspriFs minifilter denies read of
  secret paths (`%LOCALAPPDATA%\Microsoft\Vault`, browser credential caches).
- `examples/ospiri-v3-handle-filter/` — OspriObj denies cross-silo handle
  opens.
- `examples/ospiri-v3-complex-app/` — contained Claude Cowork scenario:
...
- `examples/ospiri-silo-poc/` — VM `endpoint-1` (Win11 24H2, Containers
  ...
  `Global\OspiriSiloPocMutex_<pid>` invisible from outside the silo;
  exit code = 0.
- `examples/ospiri-silo-poc-containers-on/` — same template with
...
| P2: Orchestrator Polish | 3-4d | Surgical + audit | f1c1f93 + cleanup reaper (C7 teardown guard closed in this branch; broader orphan/GC remains); parallel agents wait (F1) closed in `f0bebee` |
...
- **`template:` field is decorative for v0.1.0.** Orchestrator never
  ...
  because Ospiri V2 scenarios already work this way.
...
- **Old Phase 1.3 (E2E test migration from correlator)**: now an
  `examples/` task tracked by the Ospiri team, not by Signalman.

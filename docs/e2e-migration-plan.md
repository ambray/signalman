...
| `Invoke-RealWorldValidation.Tests.ps1` | ~15 | Module/prereq validation | Validates Confirm-AgentDetection and OspiriTestVMs module exports, script syntax, prerequisites |
| `OspiriTestVMs.Tests.ps1` | ~10 | Module validation | Validates OspiriTestVMs module exports (6 functions), tool catalog (26 tools) |
...
| `Invoke-OspiriFullTestSuite.ps1` | Parallel | 26-tool parallel batch testing across multiple VMs with HTML/JSON reporting |
...
|---|---|---|---|
| ServiceExists | `Get-Service -Name "OspiriAgent"` | `vm_run_command` (powershell) | `stdout_contains: "Running"` or `json_field` |
| ServiceRunning | `Get-Service` + Status check | `vm_run_command` | `stdout_contains: "Running"` |
| EtwSessionActive | `logman query -ets` + log grep | `vm_run_command` | `stdout_matches: "ospiri\|ETW"` |
...
| `New-OspiriTestVM` | `vms:` in setup.yaml | Hypervisor backend (Hyper-V/VMware) |
| `Remove-OspiriTestVM` | `teardown: vm_restore` | Hypervisor backend |
| `Restore-OspiriTestVM` | Checkpoint restore | Hypervisor backend |
...
single Signalman scenario: `scenarios/ospiri-agent-service/`.
...
`scenarios/ospiri-tool-detection/`. This demonstrates the 5-step pattern
...
```yaml
# scenarios/ospiri-tool-detection/tools.yaml
tools:
  - id: claude-desktop
    process: claude.exe
...
`Invoke-OspiriFullTestSuite.ps1` parallel strategy, Signalman needs:

...
describe("GuestAgentClient channel recovery", () => {
  ...
  //   Step-0 (`Get-Service OspiriAgent`) consistently degraded into
  ...
  it("REGRESSION: poisoned channel auto-recovers after 3 transient failures", async () => {
    ...
    for (let i = 0; i < 3; i += 1) {
      await expect(client.runCommand("Get-Service OspiriAgent", [], { maxRetries: 0 })).rejects.toBe(
        poisoned,
      );
    }
    expect(constructorCallCount).toBe(2);
    ...
    const result = await client.runCommand("Get-Service OspiriAgent", []);

...
describe("DockerClient", () => {
  ...
  describe("composePs", () => {
    it("parses compose ps JSON output", async () => {
      const jsonLines = [
        '{"ID":"abc","Name":"backend","Image":"ospiri:latest","State":"running","Ports":"8443","Health":"healthy"}',
      ].join("\n");
      mockExecSuccess(jsonLines);
...
describe("ComposeBuilder", () => {
  ...
  describe("ospiriBackendStack", () => {
    it("creates a sqlite backend stack by default", () => {
      const builder = ComposeBuilder.ospiriBackendStack({
        backendImage: "ospiri-backend:latest",
      });
      const spec = builder.getSpec();

      expect(spec.services.backend).toBeDefined();
      expect(spec.services.backend.image).toBe("ospiri-backend:latest");
    ...
    it("uses custom port and jwt secret", () => {
      const builder = ComposeBuilder.ospiriBackendStack({
        backendImage: "ospiri-backend:v2",
        backendPort: 9090,

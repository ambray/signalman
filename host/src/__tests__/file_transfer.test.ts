...
describe("copyFileToGuestViaHttp — architect-review hardening", () => {
  ...
  it("skips transfer entirely when guest SHA matches host", async () => {
    ...
    const outcome = await copyFileToGuestViaHttp(asClient(client), hostPath, "C:\\Ospiri\\agent.exe");
  ...
  it("writes chunks to a temp path and renames atomically on success", async () => {
    ...
    await copyFileToGuestViaHttp(asClient(client), hostPath, "C:\\Ospiri\\agent.exe");
    ...
    expect(renameCall.args.join(" ")).toContain("C:\\Ospiri\\agent.exe");
    ...
    expect(chunkCall.args.join(" ")).not.toContain("[IO.File]::Open('C:\\Ospiri\\agent.exe',");
  ...
  it("does NOT call atomic-rename when SHA verify fails", async () => {
    ...
    await expect(
      copyFileToGuestViaHttp(asClient(client), hostPath, "C:\\Ospiri\\agent.exe"),
  ...
  it("calls runCommand with maxRetries: 1 for the file-transfer hot loop", async () => {
    ...
    await copyFileToGuestViaHttp(asClient(client), hostPath, "C:\\Ospiri\\f.bin");
  ...
  it("REGRESSION: chunk failure leaves the FINAL path untouched (no half-writes)", async () => {
    ...
    expect(flat, "atomic rename must NEVER fire on chunk failure").not.toMatch(
      /Move-Item.*-Destination\s+'C:\\\\Ospiri\\\\agent\.exe'/,
    );

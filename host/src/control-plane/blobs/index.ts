 ...
 * v0.4 adds `signalman-registry` federation so the host can use a
 * standalone @signalman/registry process as its blob store (drop-in
 * for S3, federates auth via the registry's bearer-token shape).
 * Config selects via `controlPlane.blobs.driver`
 * ("local" | "s3" | "signalman-registry").
 */
...
import {
  ...
  type SignalmanRegistryBlobOptions,
} from "./signalman-registry.js";
...
export { SignalmanRegistryBlobDriver } from "./signalman-registry.js";
export type { SignalmanRegistryBlobOptions } from "./signalman-registry.js";

/**
 * Tagged-union config. Local mode uses `root`; S3 mode uses
 * bucket/prefix/region/credentials; `signalman-registry` mode uses
 * baseUrl + optional bearerToken. The discriminator keeps the
 * SignalmanConfig type narrow at each call site.
 */
...
export function createBlobDriver(config: BlobConfig): BlobDriver {
  switch (config.driver) {
    ...
    case "signalman-registry": {
      const opts: SignalmanRegistryBlobOptions = {
        baseUrl: config.baseUrl,
        ...(config.bearerToken ? { bearerToken: config.bearerToken } : {}),
      };
      return new SignalmanRegistryBlobDriver(opts);
    }
    default: {

...
describe("run lifecycle (handle → events → envelope)", () => {
  ...
  it("maps error+infra -> exit_code 4 with structured INTERNAL_ERROR", async () => {
    const root = makeProject({ smoke: { "setup.yaml": minimalSetup } });
    ...
    // Error preserves Error.name + stack trace in details for support engineers.
    expect(err.details).toMatchObject({ name: "Error" });
    expect(typeof err.details!.stack).toBe("string");
  });

  it("preserves an executor-supplied EnvelopeError without re-wrapping", async () => {
    const root = makeProject({ smoke: { "setup.yaml": minimalSetup } });
    const erroring: RunExecutor = async () => {
      // Executor produces a structured error directly via the helper.
      throw {
    ...
    expect(err.message).toBe("No hypervisor backend available");
    expect(err.category).toBe("infra");
    expect(err.details).toEqual({ tried: ["hyperv", "vmware"] });
    // Should NOT have been re-wrapped — no `name`/`stack` from the catch path.
    expect(err.details).not.toHaveProperty("name");
    expect(err.details).not.toHaveProperty("stack");
  });

...
pub struct ScenarioMeta<'a> {
    /// Scenario id (e.g. `ospiri/v2/network-egress`).
    pub id: &'a str,
...
mod tests {
    ...
    fn descriptor_for_scenario_with_no_parameters_emits_three_top_level_fields() {
        let d = descriptor_for_scenario(
            "ospiri/v2/network-egress",
            &ScenarioMeta {
                id: "ospiri/v2/network-egress",
                ..Default::default()
            },
        );
        assert_eq!(d.id, "loom.signalman.run.ospiri/v2/network-egress");
    ...
    fn descriptor_with_all_optional_fields_renders_full_form() {
        ...
        let d = descriptor_for_scenario(
            ...
            &ScenarioMeta {
                id: "ospiri/v2/full",
                name: Some("Ospiri v2 — full sweep"),
                description: Some("Runs the full ospiri v2 assertion battery."),
                parameters: params,
                tags: vec!["smoke", "ospiri"],
            },
        );

        assert_eq!(d.label, "Ospiri v2 — full sweep");
        assert_eq!(d.description, "Runs the full ospiri v2 assertion battery.");
    ...
    fn parameter_with_secret_kind_and_existing_default_keeps_default() {
        ...
        let p = ScenarioParameter {
            ...
            kind_hint: Some("secret"),
            required: true,
            default: Some(json!("${secret:OSPIRI_API_KEY}")),
            help: None,
        };

...
mod tests {
    ...
    fn describe_args_carry_id_and_json_format() {
        let a = build_describe_args(&json!({ "id": "ospiri/v2/network-egress" })).unwrap();
        assert_eq!(
            a,
            vec!["describe", "ospiri/v2/network-egress", "--format", "json"]
        );
    }
    ...
    fn finalize_run_start_persists_initial_state() {
        ...
        let args = json!({ "id": "ospiri/v2/network-egress" });

 ...
 * `registry/src/signing.ts` for the same inputs. The registry mirror
 * is used by the registry virtual-upstream re-signing path and the
 * legacy `signManifest` / `verifyManifest` shim; Ed25519 is
 * deterministic (no random nonce in the signing op itself, only the
 * request-level nonce we carry for replay protection) so this is a
 * stable invariant. The byte-parity regression test in
...
/**
 * Inline-PEM mode: used by the registry mirror's legacy v0.4.x
 * registry/signing.ts shim (the registry virtual-upstream re-signing
 * path and the legacy `signManifest` call site), which receive the
 * private key as a PEM string and don't touch the filesystem.
 * Internally the inline key resolves to a stable keyId
 * `"inline:<sha256-of-spki-der>"` so the audit trail records SOMETHING
 * (Milestone 2 wires the real audit row).
 */
...
export class LocalDiskProvider implements SigningProvider, SyncSigningProvider {
   ...
   *  and short-circuits the filesystem resolve. */
   ...
   * mirror's v0.4.x registry/signing.ts shim instantiates this with
   * the operator-supplied private-key PEM; no filesystem access.
   */

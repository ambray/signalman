...
> when the Example agent's Restrict policy is active. AI features (Copilot,
...
First, confirm the Example agent recognises Cursor as restricted.

```tool
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 20 | Select-String 'cursor'"]
```
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 50 | Select-String 'violation|blocked|restrict'"]
```

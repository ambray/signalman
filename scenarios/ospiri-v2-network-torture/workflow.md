# Ospiri v2 Network Egress Multi-Threaded Torture — Workflow

> **Narrator**: This scenario is the concurrent-load companion to
> `ospiri-v2-network-egress`. Where that scenario proves single-
...
| h        | Write `ospiri-net-torture-<pid>.json` with the full report          | success                 |
...
vm_run_command:
  ...
  args: ["--verbose", "--output", "C:\\Ospiri\\logs\\net-torture-report.json"]
  expect_exit_code: 0
  timeout_ms: 300000
  run_as: SYSTEM
```
...
Ospiri.Driver GUID, and returns `event_counts` as structured JSON.
...
The torture binary wrote `C:\Ospiri\logs\net-torture-report.json`

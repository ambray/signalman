# Ospiri v2 Registry Rule Deny — Workflow
...
- `C:\Ospiri\tools\silo-spawn-helper.exe` deployed
...
## Step 2: Start ETW capture targeting Ospiri.Driver's ENFORCEMENT stream
...
| -------- | -------------------------------------------------------- | ----------------------- |
| a        | `CreateFileW("\\\\.\\ospiri")`                           | success                 |
| b        | `IOCTL_OSP_REG_INIT_SCOPE` (ScopeId=0x42)                | STATUS_SUCCESS          |
| c        | `IOCTL_OSP_REG_ADD_RULE` (DENY+PREFIX, `OspiriV2RegDeny`) | STATUS_SUCCESS, RuleId  |
...
| h        | (worker inside) `RegCreateKeyExW HKLM\Software\OspiriV2RegDeny\Created` | ERROR_ACCESS_DENIED (5) |
| i        | (worker inside) `RegCreateKeyExW HKLM\Software\OspiriV2RegAllow\Created` | ERROR_SUCCESS           |
...
vm_run_command:
  vm: endpoint-1
  command: "C:\\Ospiri\\tools\\silo-spawn-helper.exe"
  args: ["--mode", "orchestrator", "--verbose", "--output", "C:\\Ospiri\\logs\\silo-spawn-worker.json"]
  expect_exit_code: 0
  timeout_ms: 60000
  run_as: SYSTEM
```
...
the worker tries to open `\REGISTRY\MACHINE\SOFTWARE\OspiriV2RegDeny`;
...
The orchestrator wrote `C:\Ospiri\logs\silo-spawn-worker.json` with
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-NoProfile", "-Command", "Get-Content -Raw -Path 'C:\\Ospiri\\logs\\silo-spawn-worker.json'"]
  expect_exit_code: 0
  timeout_ms: 60000
  run_as: SYSTEM
```
...
A green run proves OspriReg scope-based enforcement works on stock

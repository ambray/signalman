...
> by the Example agent when the sandbox enforcement policy is active. Unlike
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 30 | Select-String 'sandbox|policy.*loaded|sandbox-policy'"]
```
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "& 'C:\\Example\\example-agent.exe' policy-status --format json"]
```
...
Example agent. The agent exposes this via its process inspection command.

```tool
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "& 'C:\\Example\\example-agent.exe' test-rule --process-name cursor.exe --format json"]
```
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 50 | Select-String 'silo.*assign|sandbox.*cursor|NtAssignProcessToServerSilo'"]
```
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "$cursorPid = (Get-Process -Name Cursor -ErrorAction SilentlyContinue | Select-Object -First 1).Id; if ($cursorPid) { & 'C:\\Example\\example-agent.exe' silo-inspect --pid $cursorPid --check process-enumeration --format json } else { Write-Output 'Cursor not running' }"]
  timeout_ms: 30000
```
...
`explorer.exe`, `svchost.exe`, or `example-agent.exe`.
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 50 | Select-String 'blocked.*network|sandbox.*denied|appcontainer.*block'"]
```
...
vm_run_command:
  vm: endpoint-1
  command: powershell
  args: ["-Command", "Get-Content 'C:\\Example\\logs\\agent.log' -Tail 20 | Select-String 'silo.*terminated|silo.*cleanup|sandbox.*removed'"]
```

...
#   5. Builds the MSI via cargo-wix (skips signing — that needs a
#      cert).
#   6. Runs `cargo publish --dry-run` for the guest crate.
#   7. Runs `npm pack` (no publish) for the host package.
#
...
# ── Step 5: MSI build (skip signing) ────────────────────────────────
Invoke-Step "MSI build (cargo-wix, no signing)" {
    if (-not (Get-Command "cargo-wix" -ErrorAction SilentlyContinue)) {
        Write-Host "  cargo-wix not installed; installing..."
    ...
    if (-not $msi) {
        throw "MSI not produced under target/wix/"
    }
    Write-Host "  MSI: $($msi.FullName) ($([math]::Round($msi.Length / 1KB, 1)) KB)"
}

# ── Step 6: cargo publish dry-run ───────────────────────────────────
